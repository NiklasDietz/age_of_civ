...
// gini(), triangleReward(), normalisedEntropy() now live in BalanceMetrics.hpp
// (header-only, aoc::ga scope) so unit tests can exercise them directly. They
// are still called unqualified here because they share the aoc::ga namespace.
...
void evaluateBalanceIndividual(BalanceIndividual& ind,
    ...
    // Aggregate.
    std::array<int32_t, aoc::sim::VICTORY_TYPE_COUNT> victoryHist{};  // indexed by VictoryType value
    int32_t valid = 0;
    int32_t decisive = 0;
    double lenSum = 0.0;
    ...
    for (int32_t k = 0; k < cfg.gamesPerEval; ++k) {
        ...
        const std::size_t vtIdx = std::min<std::size_t>(aoc::sim::VICTORY_TYPE_COUNT - 1,
