...
TEST_CASE("every unit whose upgradesTo is set can actually be upgraded") {
    ...
    for (int32_t i = 0; i < UNIT_TYPE_COUNT; ++i) {
        const UnitTypeId id{static_cast<uint16_t>(i)};
        const UnitTypeId next = unitTypeDef(id).upgradesTo;
        if (!next.isValid()) {
            continue;
        }
        ++withSuccessor;

        // The successor must be a real row.
        REQUIRE(next.value < UNIT_TYPE_COUNT);
...
TEST_CASE("a unit with no successor offers no upgrade") {
    for (int32_t i = 0; i < UNIT_TYPE_COUNT; ++i) {
        const UnitTypeId id{static_cast<uint16_t>(i)};
        if (unitTypeDef(id).upgradesTo.isValid()) {
            continue;
        }
        CHECK(aoc::sim::getAvailableUpgrades(id).empty());
    }
}
...
TEST_CASE("no two rows claim the same upgrade, and no chain loops") {
    ...
    for (int32_t i = 0; i < UNIT_TYPE_COUNT; ++i) {
        ...
        CHECK(seenSources.insert(id.value).second);
    }

    // Walking any chain must terminate. A cycle would hang the upgrade UI.
    for (int32_t i = 0; i < UNIT_TYPE_COUNT; ++i) {
        UnitTypeId cur{static_cast<uint16_t>(i)};
        ...
        while (unitTypeDef(cur).upgradesTo.isValid() && steps < UNIT_TYPE_COUNT + 1) {
            ...
            REQUIRE(cur.value < UNIT_TYPE_COUNT);
...
TEST_CASE("a unit never upgrades into itself or backwards in era") {
    for (int32_t i = 0; i < UNIT_TYPE_COUNT; ++i) {
        const UnitTypeId id{static_cast<uint16_t>(i)};
        const aoc::sim::UnitTypeDef& def = unitTypeDef(id);
        if (!def.upgradesTo.isValid()) {
            continue;
        }
        CHECK(def.upgradesTo != id);
