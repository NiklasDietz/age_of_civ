...
TEST_CASE("war needs contact and no lock; peace waits ten turns and re-arms the lock") {
    ...
    CHECK(aoc::sim::requestDeclareWar(f.world.gameState, f.d, PlayerId{0}, PlayerId{2},
                                      CasusBelliType::SurpriseWar, 10) == ErrorCode::InvalidState); // unmet
    CHECK(aoc::sim::requestDeclareWar(f.world.gameState, f.d, PlayerId{0}, PlayerId{0},
                                      CasusBelliType::SurpriseWar, 10) == ErrorCode::EntityNotFound);
    ...
    CHECK(aoc::sim::requestDeclareWar(f.world.gameState, f.d, PlayerId{0}, PlayerId{1},
                                      CasusBelliType::SurpriseWar, 11) == ErrorCode::InvalidState); // already

    CHECK(aoc::sim::requestMakePeace(f.world.gameState, f.d, PlayerId{0}, PlayerId{1}, 15)
          == ErrorCode::InvalidState); // too early
    CHECK(aoc::sim::requestMakePeace(f.world.gameState, f.d, PlayerId{0}, PlayerId{1}, 20) == ErrorCode::Ok);
    ...
    CHECK(aoc::sim::requestDeclareWar(f.world.gameState, f.d, PlayerId{0}, PlayerId{1},
                                      CasusBelliType::SurpriseWar, 25) == ErrorCode::InvalidState); // peace lock
...
TEST_CASE("each casus belli needs its justification") {
    ...
    std::vector<CasusBelliType> cbs = aoc::sim::availableCasusBelli(gs, f.d, PlayerId{0}, PlayerId{1}, 10);
    CHECK(cbs.size() == 1);
    CHECK(Fixture::contains(cbs, CasusBelliType::SurpriseWar));
    CHECK(aoc::sim::requestDeclareWar(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::FormalWar, 10)
    ...
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::FormalWar, 20)
          == ErrorCode::Ok);
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::FormalWar, 41)
          == ErrorCode::InvalidArgument);
    // Colonial: two eras ahead.
    gs.player(PlayerId{0})->era().currentEra = aoc::EraId{2};
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::ColonialWar, 20)
          == ErrorCode::Ok);
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{1}, PlayerId{0}, CasusBelliType::ColonialWar, 20)
          == ErrorCode::InvalidArgument);
    // Economic: the target embargoes the actor.
    f.d.setEmbargo(PlayerId{1}, PlayerId{0}, true);
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::EconomicWar, 20)
          == ErrorCode::Ok);
    ...
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::ReconquestWar, 20)
          == ErrorCode::Ok);
    aoc::game::City& allied = aoc::test::addCityAt(f.world, PlayerId{1}, 18, 6, "Allied");
    allied.setOriginalOwner(PlayerId{2});
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::LiberationWar, 20)
    ...
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::HolyWar, 20)
          == ErrorCode::InvalidArgument);
    gs.player(PlayerId{0})->cities().front()->religion().pressure[0] = 10.0f;
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::HolyWar, 20)
          == ErrorCode::Ok);
    CHECK(aoc::sim::casusBelliUsable(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::ProtectorateWar, 20)
          == ErrorCode::InvalidArgument);
    ...
    CHECK(aoc::sim::requestDeclareWar(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::ReconquestWar, 20)
          == ErrorCode::Ok);
...
TEST_CASE("denouncement and friendship exclude each other and expire") {
    ...
    CHECK(aoc::sim::requestDeclareFriendship(gs, f.d, PlayerId{0}, PlayerId{1}, 10) == ErrorCode::Ok);
    CHECK(f.d.relation(PlayerId{1}, PlayerId{0}).friendshipUntilTurn == 10 + aoc::sim::FRIENDSHIP_TURNS);
    CHECK(f.hasModifier(PlayerId{1}, PlayerId{0}, "Declaration of Friendship"));
    CHECK(aoc::sim::requestDeclareWar(gs, f.d, PlayerId{0}, PlayerId{1}, CasusBelliType::SurpriseWar, 20)
          == ErrorCode::InvalidState); // friends do not fight
    CHECK(aoc::sim::requestDeclareWar(gs, f.d, PlayerId{1}, PlayerId{0}, CasusBelliType::SurpriseWar, 20)
          == ErrorCode::InvalidState);
    CHECK(aoc::sim::requestDenounce(gs, f.d, PlayerId{0}, PlayerId{1}, 20) == ErrorCode::InvalidState);
    ...
    CHECK(aoc::sim::requestDenounce(gs, f.d, PlayerId{0}, PlayerId{1}, 41) == ErrorCode::InvalidState);
...
TEST_CASE("delegation and embassy cost gold, raise intelligence and cannot repeat") {
    ...
    CHECK(aoc::sim::requestSendDelegation(gs, f.d, PlayerId{0}, PlayerId{1}) == ErrorCode::InsufficientResources);
    me.addGold(100);
    ...
    CHECK(aoc::sim::requestSendDelegation(gs, f.d, PlayerId{0}, PlayerId{1}) == ErrorCode::InvalidState);

    f.d.addModifier(PlayerId{0}, PlayerId{1}, {"Test insult", -60, 0});
    CHECK(aoc::sim::requestEstablishEmbassy(gs, f.d, PlayerId{0}, PlayerId{1}) == ErrorCode::InvalidState); // hostile
    ...
    CHECK(aoc::sim::requestEstablishEmbassy(gs, f.d, PlayerId{0}, PlayerId{1}) == ErrorCode::InvalidState);
}

TEST_CASE("open borders need consent, last thirty turns and expire on schedule") {
    Fixture f;
    aoc::game::GameState& gs = f.world.gameState;
    CHECK(aoc::sim::requestOpenBorders(gs, f.d, PlayerId{0}, PlayerId{1}, 10) == ErrorCode::InvalidState);
    f.d.addModifier(PlayerId{0}, PlayerId{1}, {"Test goodwill", 15, 0});
    ...
    CHECK(f.d.relation(PlayerId{1}, PlayerId{0}).openBordersUntilTurn == 10 + aoc::sim::OPEN_BORDERS_TURNS);
    CHECK(aoc::sim::requestOpenBorders(gs, f.d, PlayerId{0}, PlayerId{1}, 11) == ErrorCode::InvalidState);
...
TEST_CASE("an AI refuses peace while it is winning and accepts when outnumbered") {
    ...
    REQUIRE(aoc::sim::requestDeclareWar(gs, f.d, PlayerId{1}, PlayerId{0}, CasusBelliType::SurpriseWar, 10)
            == ErrorCode::Ok);
    CHECK_FALSE(aoc::sim::aiAcceptsPeace(gs, PlayerId{1}, PlayerId{0}));
    CHECK(aoc::sim::requestMakePeace(gs, f.d, PlayerId{0}, PlayerId{1}, 30) == ErrorCode::InvalidState);
