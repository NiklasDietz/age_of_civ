...
/**
 * @file HistoricMomentsScreen.hpp
 * @brief Read-only Historic Moments screen: the player's current age, era score
 *        against the golden and dark thresholds, lifetime score, and the timeline
 *        of every era-score award (newest first). Rebuilds only when the timeline
 *        or the age moves.
 */
...
class HistoricMomentsScreen final : public ScreenBase {
public:
...
private:
    ...
    /// Cheap change detector so `refresh` rebuilds only when the timeline moved.
