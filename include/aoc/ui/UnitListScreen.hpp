...
/**
 * @file UnitListScreen.hpp
 * @brief Every own unit on one screen: type, position, health, movement left
 *        and state, with a Go button that centres the camera and a Select
 *        button that selects the unit. Civ VI plan Phase 2.3, 2026-09-05.
 */
...
class UnitListScreen final : public ScreenBase {
public:
    ...
    /// `onJump` centres the camera on a unit, `onSelect` selects it on the map.
    void setCallbacks(LocationCallback onJump, LocationCallback onSelect);
