...
enum class VictoryMode : uint8_t {
    ...
    Classic, ///< Traditional win conditions: Domination, Science, Culture, Religion, Diplomatic,
             ///< Score
};
...
public:
     ...
     */
    void build(UIManager& ui, float screenW, float screenH, std::function<void()> onStartGame,
               ...
               std::function<void()> onMapEditor = {}, std::function<void()> onLoadGame = {});
...
private:
    ...
    /// Content-panel size as built; updateLayout re-centres with these.
    float m_panelW = 0.0f;
    float m_panelH = 0.0f;
    ...
    std::function<void()> m_onLoadGame;
};
...
public:
    void build(UIManager& ui, float screenW, float screenH,
               std::function<void(const GameSetupConfig&)> onStart, std::function<void()> onBack);
    void destroy(UIManager& ui);
