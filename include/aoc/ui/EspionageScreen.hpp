...
/**
 * @file EspionageScreen.hpp
 * @brief Read-only espionage screen: the human player's spies, the missions
 *        each could run where it stands, and every rival's intelligence level.
 */
...
class EspionageScreen final : public ScreenBase {
public:
...
private:
    ...
    /// Cheap change detector so `refresh` rebuilds the rows only when spy state moved.
