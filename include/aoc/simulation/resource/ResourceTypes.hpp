...
[[nodiscard]] inline constexpr ClimateBand tileClimateBand(aoc::map::TerrainType terrain,
                                                           aoc::map::FeatureType feature) {
    if (feature == aoc::map::FeatureType::Jungle) {
        return ClimateBand::Tropical;
    }
    switch (terrain) {
...
[[nodiscard]] inline constexpr int16_t defaultReserves(uint16_t goodId) {
    ...
    if (goodId <= 11) {
        switch (goodId) {
        case 0:
            return 100; // Iron ore: abundant
        case 1:
            return 80; // Copper ore: common
        case 2:
            return 120; // Coal: large deposits
        case 3:
            return 60; // Oil: moderate
        case 4:
            return -1; // Horses: renewable (breeding)
        case 5:
            return 50; // Niter: scarce
        case 6:
            return 30; // Uranium: very scarce
        case 7:
            return 40; // Aluminum: moderate
        case 8:
            return -1; // Cotton: renewable (crop)
        case 9:
            return -1; // Rubber: renewable (trees)
        case 10:
            return 60; // Tin: moderate
        case 11:
            return 70; // Silver ore: moderate
        case 12:
            return 80; // Natural gas: moderate deposits
        default:
            return 80;
        }
    }
    ...
    if (goodId >= 20 && goodId <= 28) {
        switch (goodId) {
        case 20:
            return 50; // Gold ore: finite, scarce
        case 21:
            return 40; // Gems: finite, very scarce
        case 22:
            return -1; // Spices: renewable (plants)
        case 23:
            return -1; // Silk: renewable (silkworms)
        case 24:
            return 60; // Ivory: finite (elephants decline)
        case 25:
            return -1; // Wine: renewable (grapes)
        case 26:
            return -1; // Dyes: renewable (plants)
        case 27:
            return -1; // Furs: renewable (animals, slowly)
        case 28:
            return -1; // Incense: renewable (trees)
        case 29:
            return 200; // Salt: large finite deposits
        case 30:
            return 80; // Marble: finite quarry
        case 31:
            return -1; // Pearls: renewable (diving)
        case 32:
            return -1; // Tea: renewable (plantation)
        case 33:
            return -1; // Coffee: renewable (plantation)
        case 34:
            return -1; // Tobacco: renewable (plantation)
        default:
            return -1;
        }
    }
    ...
    if (goodId == 146) {
        return 60;
    } // LITHIUM: scarce, like niter.
    if (goodId == 150) {
        return 35;
    } // RARE_EARTH (WP-B3): very scarce.
...
[[nodiscard]] inline constexpr TechId resourceRevealTech(uint16_t goodId) {
    switch (goodId) {
    case 0:
        return TechId{4}; // Iron ore → Bronze Working
    case 1:
        return TechId{0}; // Copper ore → Mining
    case 2:
        return TechId{11}; // Coal → Industrialization
    case 3:
        return TechId{12}; // Oil → Refining  (was 13 = Economics, wrong)
    case 4:
        return TechId{1}; // Horses → Animal Husbandry
    case 5:
        return TechId{7}; // Niter → Apprenticeship
    case 6:
        return TechId{18}; // Uranium → Nuclear Fission
    case 7:
        return TechId{14}; // Aluminum → Electricity
    case 10:
...
// -- Retired monetary goods (140-142) -- IDs kept for save compatibility; no recipes produce them
inline constexpr uint16_t COPPER_COINS = 140;
inline constexpr uint16_t SILVER_COINS = 141;
inline constexpr uint16_t GOLD_BARS    = 142;
...
inline constexpr uint16_t TITANIUM =
    145; ///< Lunar ore, delivered only after Lunar Colony project; gates Mars.
...
inline constexpr uint16_t LITHIUM =
    146; ///< Raw strategic: rare-earth tile or Lunar Colony byproduct.
inline constexpr uint16_t BATTERIES =
    147; ///< Processed: from Lithium + Copper, feeds electrified industry.
inline constexpr uint16_t ELECTRICITY =
    148; ///< Processed energy (tracked power), generated at power plants.
inline constexpr uint16_t PHARMACEUTICALS =
    149; ///< Advanced: consumer-health good; population/amenity multiplier input.
inline constexpr uint16_t RARE_EARTH = 150; ///< WP-B3 raw strategic: rare Mountain deposit OR Lunar
                                            ///< Colony byproduct. Alt input for Semiconductors.
...
[[nodiscard]] inline constexpr bool isMountainMetal(uint16_t goodId) {
    return goodId == goods::IRON_ORE || goodId == goods::COPPER_ORE ||
           goodId == goods::SILVER_ORE || goodId == goods::GOLD_ORE;
}
...
struct RecipeInput {
    ...
    int32_t amount;
    bool consumed =
        true; ///< If false, the input is required but not consumed (e.g., Computers for Software).
};
