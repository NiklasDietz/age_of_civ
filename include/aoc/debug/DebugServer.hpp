 ...
 * (default 9876). Runs `httplib::Server` on its own thread; routes
 * are registered by the owner via `route()` after construction and
 * before `start()`. Routes execute on the server thread, so handlers
 * MUST avoid touching state that lives on the main / render thread.
 * Use the `MutationQueue` (separate file) to defer writes to the
 * game-loop thread, and `std::shared_ptr<const StateSnapshot>` for
 * lock-free reads.
 *
 * Lifecycle:
 *   1. Construct with port (binds nothing yet).
 *   2. `route(method, path, handler)` for every endpoint.
 *   3. `start()` -- spawns listener thread.
 *   4. Server thread runs until `stop()` is called or the process
 *      exits. Destructor calls stop().
 *
 * Threading:
 *   - Listener thread: blocks in cpp-httplib's accept loop.
 *   - Per-request: cpp-httplib uses a thread pool (default 8). Every
 *     handler call may run concurrently; handlers must be re-entrant.
 *
