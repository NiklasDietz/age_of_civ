...
/// Written into every header. Alias: bump CURRENT_SAVE_VERSION in SaveVersioning.hpp.
inline constexpr uint32_t SAVE_VERSION = CURRENT_SAVE_VERSION;
...
enum class SectionId : uint16_t {
    ...
    // v34 sections --------------------------------------------------------
    ActiveDeals        = 0x002D, ///< Deals in force (GameState::deals()); read after Entities
};
 ...
 */
[[nodiscard]] ErrorCode saveGame(const std::string& filepath, const aoc::game::GameState& gameState,
 ...
 */
[[nodiscard]] ErrorCode loadGame(const std::string& filepath, aoc::game::GameState& gameState,
