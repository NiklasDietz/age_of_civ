...
enum class ImprovementType : uint8_t {
    ...
    // -- Extended modern set --
    GeothermalVent,    ///< Mountain-adjacent flag tile: +1 faith, +1 prod. Enables GeothermalPlant
                       ///< building.
...
public:
    ...
    /// Upper bound on either grid dimension. Every entry point that takes
    /// caller-supplied dimensions must clamp against this BEFORE allocating,
    /// because width*height sizes ~50 per-tile layers plus the worldgen
    /// intermediates. The save loader already enforced it (Serializer.cpp);
    /// it lives here so the loader, the generator, the net config and the CLI
    /// share one number instead of each inventing a limit.
    static constexpr int32_t MAX_MAP_DIMENSION = 4096;
    /// Smallest grid the worldgen passes are meaningful on (several use fixed
    /// BFS radii and multi-cell windows).
    static constexpr int32_t MIN_MAP_DIMENSION = 8;
    ...
    // ========================================================================
    // Row geography (true latitude per grid row)
    // ========================================================================
    //
    // Every sphere -> rectangle projection the generator offers is cylindrical
    // or pseudocylindrical, so a row's latitude is a function of the row alone.
    // MapGenerator fills this from the SAME projection it samples terrain
    // through, which is the point: before this existed, every climate and
    // biogeography pass reconstructed latitude as `2 * |row/height - 0.5|` --
    // i.e. it assumed row was linear in latitude. No projection here makes that
    // true, so climate zones sat 8-9 degrees away from the terrain they
    // described under Mollweide, and would sit 15-19 degrees away under an
    // equal-area projection where row is linear in sin(latitude).
    //
    // DEBT: latitude, longitude and per-tile sphere area are one cohesive
    // concern (the grid's geometric embedding) and want extracting into a
    // HexGeography value type rather than accreting onto HexGrid. Row latitude
    // lands here now because it is what the ~46 existing call sites need.

    /// Latitude in degrees at the CENTRE of each grid row, north positive.
    void setRowLatitudes(std::vector<float> v) { this->m_rowLatitudeDeg = std::move(v); }

    [[nodiscard]] const std::vector<float>& rowLatitudes() const { return this->m_rowLatitudeDeg; }

    /// Latitude in degrees at the centre of `row`. Returns 0 (equator) when the
    /// table is absent, which is the least-wrong answer for a grid built
    /// without a projection (hand-built test grids).
    [[nodiscard]] float rowLatitudeDeg(int32_t row) const {
        const std::size_t idx = static_cast<std::size_t>(row);
        return (idx < this->m_rowLatitudeDeg.size()) ? this->m_rowLatitudeDeg[idx] : 0.0f;
    }

    /// |latitude| / 90, i.e. 0 at the equator and 1 at a pole.
    ///
    /// This is the replacement for the old `2 * |ny - 0.5|` idiom and keeps its
    ...
    [[nodiscard]] bool isValid(hex::OffsetCoord c) const {
        ...
        return c.col >= 0 && c.col < this->m_width && c.row >= 0 && c.row < this->m_height;
    }
    ...
    // -- Terrain --
    [[nodiscard]] TerrainType terrain(int32_t index) const {
        this->assertIndex(index);
        return this->m_terrain[static_cast<std::size_t>(index)];
    }
    void setTerrain(int32_t index, TerrainType type) {
        this->assertIndex(index);
        this->m_terrain[static_cast<std::size_t>(index)] = type;
    }

    // -- Feature --
    [[nodiscard]] FeatureType feature(int32_t index) const {
        this->assertIndex(index);
        return this->m_feature[static_cast<std::size_t>(index)];
    }
    void setFeature(int32_t index, FeatureType type) {
        this->assertIndex(index);
        this->m_feature[static_cast<std::size_t>(index)] = type;
    }

    // -- Elevation --
    [[nodiscard]] int8_t elevation(int32_t index) const {
        this->assertIndex(index);
        return this->m_elevation[static_cast<std::size_t>(index)];
    }
    void setElevation(int32_t index, int8_t elev) {
        this->assertIndex(index);
        this->m_elevation[static_cast<std::size_t>(index)] = elev;
    }

    // -- River edges (6-bit mask, one bit per hex edge) --
    [[nodiscard]] uint8_t riverEdges(int32_t index) const {
        this->assertIndex(index);
        return this->m_riverEdges[static_cast<std::size_t>(index)];
    }
    void setRiverEdges(int32_t index, uint8_t mask) {
        this->assertIndex(index);
        this->m_riverEdges[static_cast<std::size_t>(index)] = mask;
    }
    ...
    [[nodiscard]] ResourceId resource(int32_t index) const {
        this->assertIndex(index);
        return this->m_resource[static_cast<std::size_t>(index)];
    }
    void setResource(int32_t index, ResourceId id) {
        this->assertIndex(index);
        this->m_resource[static_cast<std::size_t>(index)] = id;
    }

    // -- Resource reserves (how much extractable material remains) --
    /// Remaining reserves for the tile's resource. 0 = exhausted.
    [[nodiscard]] int16_t reserves(int32_t index) const {
        this->assertIndex(index);
        return this->m_reserves[static_cast<std::size_t>(index)];
    }
    void setReserves(int32_t index, int16_t amount) {
        this->assertIndex(index);
        this->m_reserves[static_cast<std::size_t>(index)] = amount;
    }
    ...
    [[nodiscard]] int8_t prospectCooldown(int32_t index) const {
        this->assertIndex(index);
        return this->m_prospectCooldown[static_cast<std::size_t>(index)];
    }
    void setProspectCooldown(int32_t index, int8_t turns) {
        this->assertIndex(index);
        this->m_prospectCooldown[static_cast<std::size_t>(index)] = turns;
    }
    ...
    void tickProspectCooldowns() {
        for (std::size_t i = 0; i < this->m_prospectCooldown.size(); ++i) {
            if (this->m_prospectCooldown[i] > 0) {
                --this->m_prospectCooldown[i];
            }
        }
    }

    // -- Owning player --
    [[nodiscard]] PlayerId owner(int32_t index) const {
        this->assertIndex(index);
        return this->m_owner[static_cast<std::size_t>(index)];
    }
    void setOwner(int32_t index, PlayerId player) {
        this->assertIndex(index);
        this->m_owner[static_cast<std::size_t>(index)] = player;
    }

    // -- Natural wonder --
    [[nodiscard]] NaturalWonderType naturalWonder(int32_t index) const {
        this->assertIndex(index);
        return this->m_naturalWonder[static_cast<std::size_t>(index)];
    }
    void setNaturalWonder(int32_t index, NaturalWonderType type) {
        this->assertIndex(index);
        this->m_naturalWonder[static_cast<std::size_t>(index)] = type;
    }

    // -- Tile improvement --
    [[nodiscard]] ImprovementType improvement(int32_t index) const {
        this->assertIndex(index);
        return this->m_improvement[static_cast<std::size_t>(index)];
    }
    ...
    [[nodiscard]] bool hasRoad(int32_t index) const {
        this->assertIndex(index);
        return this->m_road[static_cast<std::size_t>(index)] != 0;
    }
    ...
    [[nodiscard]] bool hasCanal(int32_t index) const {
        this->assertIndex(index);
        return this->m_improvement[static_cast<std::size_t>(index)] == ImprovementType::Canal;
    }
    ...
    [[nodiscard]] int32_t infrastructureTier(int32_t index) const {
        ...
        if (imp == ImprovementType::Highway) {
            return 3;
        }
        if (imp == ImprovementType::Railway) {
            return 2;
        }
        if (imp == ImprovementType::Road) {
            return 1;
        }
        if (this->m_road[static_cast<std::size_t>(index)] != 0) {
            return 1;
        }
        return 0;
    }
    ...
    [[nodiscard]] ChokepointType chokepoint(int32_t index) const {
        this->assertIndex(index);
        return this->m_chokepoint[static_cast<std::size_t>(index)];
    }
    void setChokepoint(int32_t index, ChokepointType type) {
        this->assertIndex(index);
        this->m_chokepoint[static_cast<std::size_t>(index)] = type;
    }
    [[nodiscard]] bool isChokepoint(int32_t index) const {
        return this->chokepoint(index) != ChokepointType::None;
    }
...
private:
    ...
    std::vector<int8_t>
        m_prospectCooldown; ///< Turns until tile can be prospected again (0 = available)
...
public:
    ...
    void setHotspots(std::vector<std::pair<float, float>> hs) { this->m_hotspots = std::move(hs); }
    ...
    [[nodiscard]] const std::vector<float>& plateLandFrac() const { return this->m_plateLandFrac; }
    void setPlateLandFrac(std::vector<float> v) { this->m_plateLandFrac = std::move(v); }
    /// Per-plate crust age in My.
    [[nodiscard]] const std::vector<float>& plateCrustAge() const { return this->m_plateCrustAge; }
    void setPlateCrustAge(std::vector<float> v) { this->m_plateCrustAge = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& plateIsPolar() const { return this->m_plateIsPolar; }
    void setPlateIsPolar(std::vector<uint8_t> v) { this->m_plateIsPolar = std::move(v); }
    ...
    void setBoundaryTypeTile(int32_t index, uint8_t bt) {
        const int32_t total = this->tileCount();
        if (this->m_boundaryTypeTile.empty()) {
            this->m_boundaryTypeTile.assign(static_cast<std::size_t>(total), 0u);
        }
        if (index >= 0 && index < total) {
    ...
    [[nodiscard]] const std::vector<float>& crustAgeTile() const { return this->m_crustAgeTile; }
    void setCrustAgeTile(std::vector<float> v) { this->m_crustAgeTile = std::move(v); }
    ...
    void setMagneticPolarity(std::vector<int8_t> v) { this->m_magneticPolarity = std::move(v); }
    ...
    [[nodiscard]] const std::vector<float>& sedimentDepth() const { return this->m_sedimentDepth; }
    void setSedimentDepth(std::vector<float> v) { this->m_sedimentDepth = std::move(v); }
    ...
    [[nodiscard]] const std::vector<float>& soilFertility() const { return this->m_soilFertility; }
    void setSoilFertility(std::vector<float> v) { this->m_soilFertility = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& volcanism() const { return this->m_volcanism; }
    void setVolcanism(std::vector<uint8_t> v) { this->m_volcanism = std::move(v); }
    ...
    void setSeismicHazard(std::vector<uint8_t> v) { this->m_seismicHazard = std::move(v); }
    ...
    void setClimateHazard(std::vector<uint8_t> v) { this->m_climateHazard = std::move(v); }
    ...
    void setGlacialFeature(std::vector<uint8_t> v) { this->m_glacialFeature = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& oceanZone() const { return this->m_oceanZone; }
    void setOceanZone(std::vector<uint8_t> v) { this->m_oceanZone = std::move(v); }

    /// Per-tile cloud-cover proxy 0..1. Computed from moisture × temp;
    /// not used for any game effect, drives a visual overlay only.
    [[nodiscard]] const std::vector<float>& cloudCover() const { return this->m_cloudCover; }
    void setCloudCover(std::vector<float> v) { this->m_cloudCover = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& flowDir() const { return this->m_flowDir; }
    void setFlowDir(std::vector<uint8_t> v) { this->m_flowDir = std::move(v); }
    ...
    void setNaturalHazard(std::vector<uint16_t> v) { this->m_naturalHazard = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& biomeSubtype() const { return this->m_biomeSubtype; }
    void setBiomeSubtype(std::vector<uint8_t> v) { this->m_biomeSubtype = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& marineDepth() const { return this->m_marineDepth; }
    void setMarineDepth(std::vector<uint8_t> v) { this->m_marineDepth = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& wildlife() const { return this->m_wildlife; }
    void setWildlife(std::vector<uint8_t> v) { this->m_wildlife = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& disease() const { return this->m_disease; }
    void setDisease(std::vector<uint8_t> v) { this->m_disease = std::move(v); }

    /// Per-tile renewable-energy potential 0-255.
    [[nodiscard]] const std::vector<uint8_t>& windEnergy() const { return this->m_windEnergy; }
    void setWindEnergy(std::vector<uint8_t> v) { this->m_windEnergy = std::move(v); }
    [[nodiscard]] const std::vector<uint8_t>& solarEnergy() const { return this->m_solarEnergy; }
    void setSolarEnergy(std::vector<uint8_t> v) { this->m_solarEnergy = std::move(v); }
    [[nodiscard]] const std::vector<uint8_t>& hydroEnergy() const { return this->m_hydroEnergy; }
    void setHydroEnergy(std::vector<uint8_t> v) { this->m_hydroEnergy = std::move(v); }
    [[nodiscard]] const std::vector<uint8_t>& geothermalEnergy() const {
        return this->m_geothermalEnergy;
    }
    void setGeothermalEnergy(std::vector<uint8_t> v) { this->m_geothermalEnergy = std::move(v); }
    [[nodiscard]] const std::vector<uint8_t>& tidalEnergy() const { return this->m_tidalEnergy; }
    void setTidalEnergy(std::vector<uint8_t> v) { this->m_tidalEnergy = std::move(v); }
    [[nodiscard]] const std::vector<uint8_t>& waveEnergy() const { return this->m_waveEnergy; }
    void setWaveEnergy(std::vector<uint8_t> v) { this->m_waveEnergy = std::move(v); }
    ...
    void setAtmosphericExtras(std::vector<uint8_t> v) { this->m_atmosphericExtras = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& hydroExtras() const { return this->m_hydroExtras; }
    void setHydroExtras(std::vector<uint8_t> v) { this->m_hydroExtras = std::move(v); }
    ...
    void setDefensibility(std::vector<uint8_t> v) { this->m_defensibility = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& domesticable() const { return this->m_domesticable; }
    void setDomesticable(std::vector<uint8_t> v) { this->m_domesticable = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& habitability() const { return this->m_habitability; }
    void setHabitability(std::vector<uint8_t> v) { this->m_habitability = std::move(v); }
    ...
    void setWetlandSubtype(std::vector<uint8_t> v) { this->m_wetlandSubtype = std::move(v); }

    /// Per-tile coral reef tier.
    /// 0 none, 1 fringing reef, 2 barrier reef, 3 atoll, 4 patch reef.
    [[nodiscard]] const std::vector<uint8_t>& reefTier() const { return this->m_reefTier; }
    void setReefTier(std::vector<uint8_t> v) { this->m_reefTier = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& warDamageAll() const { return this->m_warDamage; }
    ...
    /// arctic).
    [[nodiscard]] const std::vector<uint8_t>& koppen() const { return this->m_koppen; }
    void setKoppen(std::vector<uint8_t> v) { this->m_koppen = std::move(v); }
    ...
    void setMountainStructure(std::vector<uint8_t> v) { this->m_mountainStructure = std::move(v); }
    ...
    void setStrait(std::vector<uint8_t> v) { this->m_strait = std::move(v); }

    /// Per-tile natural-harbor score 0-255 (coastal indentation +
    /// island protection). High = sheltered deepwater harbor.
    [[nodiscard]] const std::vector<uint8_t>& harborScore() const { return this->m_harborScore; }
    void setHarborScore(std::vector<uint8_t> v) { this->m_harborScore = std::move(v); }
    ...
    void setChannelPattern(std::vector<uint8_t> v) { this->m_channelPattern = std::move(v); }
    ...
    void setVegetationDensity(std::vector<uint8_t> v) { this->m_vegetationDensity = std::move(v); }
    ...
    void setCoastalFeature(std::vector<uint8_t> v) { this->m_coastalFeature = std::move(v); }
    ...
    void setSubmarineVent(std::vector<uint8_t> v) { this->m_submarineVent = std::move(v); }
    ...
    void setVolcanicProfile(std::vector<uint8_t> v) { this->m_volcanicProfile = std::move(v); }

    /// Per-tile karst subtype.
    /// 0 = none, 1 = doline (sinkhole), 2 = polje (large flat plain),
    /// 3 = tower karst, 4 = pavement, 5 = dry valley.
    [[nodiscard]] const std::vector<uint8_t>& karstSubtype() const { return this->m_karstSubtype; }
    void setKarstSubtype(std::vector<uint8_t> v) { this->m_karstSubtype = std::move(v); }
    ...
    void setDesertSubtype(std::vector<uint8_t> v) { this->m_desertSubtype = std::move(v); }

    /// Per-tile mass-wasting type.
    /// 0 = none, 1 = rockfall, 2 = slump, 3 = debris flow, 4 = mudflow,
    /// 5 = solifluction (periglacial), 6 = rock glacier.
    [[nodiscard]] const std::vector<uint8_t>& massWasting() const { return this->m_massWasting; }
    void setMassWasting(std::vector<uint8_t> v) { this->m_massWasting = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& namedWind() const { return this->m_namedWind; }
    void setNamedWind(std::vector<uint8_t> v) { this->m_namedWind = std::move(v); }
    ...
    void setForestAgeClass(std::vector<uint8_t> v) { this->m_forestAgeClass = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& lithology() const { return this->m_lithology; }
    void setLithology(std::vector<uint8_t> v) { this->m_lithology = std::move(v); }
    ...
    /// 14 anthrosol.
    [[nodiscard]] const std::vector<uint8_t>& soilOrder() const { return this->m_soilOrder; }
    void setSoilOrder(std::vector<uint8_t> v) { this->m_soilOrder = std::move(v); }
    ...
    void setCrustalThickness(std::vector<uint8_t> v) { this->m_crustalThickness = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& albedo() const { return this->m_albedo; }
    void setAlbedo(std::vector<uint8_t> v) { this->m_albedo = std::move(v); }
    ...
    void setAtmosphericRiver(std::vector<uint8_t> v) { this->m_atmosphericRiver = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& cycloneBasin() const { return this->m_cycloneBasin; }
    void setCycloneBasin(std::vector<uint8_t> v) { this->m_cycloneBasin = std::move(v); }
    ...
    void setSeaSurfaceTemp(std::vector<uint8_t> v) { this->m_seaSurfaceTemp = std::move(v); }
    ...
    void setBedrockLithology(std::vector<uint8_t> v) { this->m_bedrockLithology = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& cliffCoastAll() const { return this->m_cliffCoast; }
    void setCliffCoast(std::vector<uint8_t> v) { this->m_cliffCoast = std::move(v); }
    ...
    void setCoastalLandform(std::vector<uint8_t> v) { this->m_coastalLandform = std::move(v); }

    /// Per-tile river regime.
    /// 0 no river, 1 perennial, 2 intermittent (seasonal), 3 ephemeral
    /// (flash-flood wadi), 4 glacier-fed, 5 snow-fed.
    [[nodiscard]] const std::vector<uint8_t>& riverRegime() const { return this->m_riverRegime; }
    void setRiverRegime(std::vector<uint8_t> v) { this->m_riverRegime = std::move(v); }

    /// Per-tile arid-erosion landform.
    /// 0 none, 1 mesa, 2 butte, 3 plateau, 4 yardang, 5 hoodoo,
    /// 6 pediment, 7 slot canyon.
    [[nodiscard]] const std::vector<uint8_t>& aridLandform() const { return this->m_aridLandform; }
    void setAridLandform(std::vector<uint8_t> v) { this->m_aridLandform = std::move(v); }
    ...
    void setLakeEffectSnow(std::vector<uint8_t> v) { this->m_lakeEffectSnow = std::move(v); }
    ...
    void setSutureReactivated(std::vector<uint8_t> v) { this->m_sutureReactivated = std::move(v); }
    ...
    void setSolarInsolation(std::vector<uint8_t> v) { this->m_solarInsolation = std::move(v); }
    ...
    void setTopographicAspect(std::vector<uint8_t> v) { this->m_topographicAspect = std::move(v); }
    ...
    void setEcotone(std::vector<uint8_t> v) { this->m_ecotone = std::move(v); }
    ...
    void setGlacialRebound(std::vector<uint8_t> v) { this->m_glacialRebound = std::move(v); }
    ...
    void setCoastalChange(std::vector<uint8_t> v) { this->m_coastalChange = std::move(v); }
    ...
    void setAquiferRecharge(std::vector<uint8_t> v) { this->m_aquiferRecharge = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& pastureScore() const { return this->m_pastureScore; }
    void setPastureScore(std::vector<uint8_t> v) { this->m_pastureScore = std::move(v); }
    ...
    void setForestryYield(std::vector<uint8_t> v) { this->m_forestryYield = std::move(v); }

    /// Per-Mountain-tile fold axis direction 0-5 (perpendicular to
    /// compression axis). 0xFF if not folded.
    [[nodiscard]] const std::vector<uint8_t>& foldAxis() const { return this->m_foldAxis; }
    void setFoldAxis(std::vector<uint8_t> v) { this->m_foldAxis = std::move(v); }
    ...
    void setMetamorphicFacies(std::vector<uint8_t> v) { this->m_metamorphicFacies = std::move(v); }
    /// Per-tile plate stress proxy 0-255 (sampled from owning plate's
    /// orogenyLocal grid at this tile's plate-local coords).
    [[nodiscard]] const std::vector<uint8_t>& plateStress() const { return this->m_plateStress; }
    void setPlateStress(std::vector<uint8_t> v) { this->m_plateStress = std::move(v); }
    ...
    void setCycloneIntensity(std::vector<uint8_t> v) { this->m_cycloneIntensity = std::move(v); }
    /// Per-tile drought severity tier 0-4.
    [[nodiscard]] const std::vector<uint8_t>& droughtSeverity() const {
        return this->m_droughtSeverity;
    }
    void setDroughtSeverity(std::vector<uint8_t> v) { this->m_droughtSeverity = std::move(v); }
    ...
    void setEndemismIndex(std::vector<uint8_t> v) { this->m_endemismIndex = std::move(v); }
    ...
    void setSpeciesRichness(std::vector<uint8_t> v) { this->m_speciesRichness = std::move(v); }
    ...
    void setGrowingSeasonDays(std::vector<uint8_t> v) { this->m_growingSeasonDays = std::move(v); }
    /// Per-tile frost days per year 0-255 (= 0-365 frost days).
    [[nodiscard]] const std::vector<uint8_t>& frostDays() const { return this->m_frostDays; }
    void setFrostDays(std::vector<uint8_t> v) { this->m_frostDays = std::move(v); }
    ...
    void setCarryingCapacity(std::vector<uint8_t> v) { this->m_carryingCapacity = std::move(v); }

    /// Per-tile soil texture fractions 0-255 (sums to ~255 across the
    /// three components). Drives drainage, nutrient retention, tilth.
    [[nodiscard]] const std::vector<uint8_t>& soilClayPct() const { return this->m_soilClayPct; }
    void setSoilClayPct(std::vector<uint8_t> v) { this->m_soilClayPct = std::move(v); }
    [[nodiscard]] const std::vector<uint8_t>& soilSiltPct() const { return this->m_soilSiltPct; }
    void setSoilSiltPct(std::vector<uint8_t> v) { this->m_soilSiltPct = std::move(v); }
    [[nodiscard]] const std::vector<uint8_t>& soilSandPct() const { return this->m_soilSandPct; }
    void setSoilSandPct(std::vector<uint8_t> v) { this->m_soilSandPct = std::move(v); }
    ...
    void setSeasonalTempRange(std::vector<uint8_t> v) { this->m_seasonalTempRange = std::move(v); }
    ...
    void setDiurnalTempRange(std::vector<uint8_t> v) { this->m_diurnalTempRange = std::move(v); }

    /// Per-tile UV-index proxy 0-255 (lat + altitude + ozone).
    [[nodiscard]] const std::vector<uint8_t>& uvIndex() const { return this->m_uvIndex; }
    void setUvIndex(std::vector<uint8_t> v) { this->m_uvIndex = std::move(v); }
    ...
    void setCoralBleachRisk(std::vector<uint8_t> v) { this->m_coralBleachRisk = std::move(v); }
    ...
    void setMagneticAnomaly(std::vector<uint8_t> v) { this->m_magneticAnomaly = std::move(v); }

    /// Per-tile heat flow refined 0-255 (refined per-tile mW/m² blending
    /// crust age + lithology + volcanism).
    [[nodiscard]] const std::vector<uint8_t>& heatFlow() const { return this->m_heatFlow; }
    void setHeatFlow(std::vector<uint8_t> v) { this->m_heatFlow = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& tsunamiRunup() const { return this->m_tsunamiRunup; }
    void setTsunamiRunup(std::vector<uint8_t> v) { this->m_tsunamiRunup = std::move(v); }
    ...
    void setTopoWetnessIndex(std::vector<uint8_t> v) { this->m_topoWetnessIndex = std::move(v); }
    /// Per-tile effective relief / roughness 0-255 (max-min elev in
    /// 3-hex neighbourhood).
    [[nodiscard]] const std::vector<uint8_t>& roughness() const { return this->m_roughness; }
    void setRoughness(std::vector<uint8_t> v) { this->m_roughness = std::move(v); }
    /// Per-tile topographic curvature.
    /// 0 flat, 1 concave (bowl, accumulation), 2 convex (ridge, divergence).
    [[nodiscard]] const std::vector<uint8_t>& curvature() const { return this->m_curvature; }
    void setCurvature(std::vector<uint8_t> v) { this->m_curvature = std::move(v); }
    ...
    void setRiverDischarge(std::vector<uint8_t> v) { this->m_riverDischarge = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& faultTrace() const { return this->m_faultTrace; }
    void setFaultTrace(std::vector<uint8_t> v) { this->m_faultTrace = std::move(v); }
    /// Per-coastal-tile reef terrace level 0-255 (Quaternary glacio-
    /// eustatic step, higher = older / higher elevation paleoshore).
    [[nodiscard]] const std::vector<uint8_t>& reefTerrace() const { return this->m_reefTerrace; }
    void setReefTerrace(std::vector<uint8_t> v) { this->m_reefTerrace = std::move(v); }
    ...
    void setMineSuitability(std::vector<uint8_t> v) { this->m_mineSuitability = std::move(v); }
    ...
    void setCoalSeamThickness(std::vector<uint8_t> v) { this->m_coalSeamThickness = std::move(v); }

    /// Per-tile soil pH 0-255 (= pH 4.0..9.0 scaled). 0 = unset, 100
    /// ≈ pH 5.5 acid, 130 ≈ pH 6.5 neutral, 180 ≈ pH 8.0 alkaline.
    [[nodiscard]] const std::vector<uint8_t>& soilPh() const { return this->m_soilPh; }
    void setSoilPh(std::vector<uint8_t> v) { this->m_soilPh = std::move(v); }
    ...
    void setIceCoverDuration(std::vector<uint8_t> v) { this->m_iceCoverDuration = std::move(v); }
    ...
    [[nodiscard]] const std::vector<uint8_t>& petIndex() const { return this->m_petIndex; }
    void setPetIndex(std::vector<uint8_t> v) { this->m_petIndex = std::move(v); }
    /// Per-tile aridity index 0-255 (PET / precipitation ratio).
    /// 0 hyper-humid, 255 hyper-arid.
    [[nodiscard]] const std::vector<uint8_t>& aridityIndex() const { return this->m_aridityIndex; }
    void setAridityIndex(std::vector<uint8_t> v) { this->m_aridityIndex = std::move(v); }
    ...
    void setErosionPotential(std::vector<uint8_t> v) { this->m_erosionPotential = std::move(v); }
    ...
    void setFloodFrequency(std::vector<uint8_t> v) { this->m_floodFrequency = std::move(v); }
    ...
    void setRiparianForest(std::vector<uint8_t> v) { this->m_riparianForest = std::move(v); }
    ...
    void setMagneticIntensity(std::vector<uint8_t> v) { this->m_magneticIntensity = std::move(v); }
    ...
    void setGroundwaterDepth(std::vector<uint8_t> v) { this->m_groundwaterDepth = std::move(v); }
...
private:
    ...
    /// Latitude (deg, north positive) at the centre of each grid row,
    /// derived from the active projection. Empty on grids built without one.
    std::vector<float> m_rowLatitudeDeg;
