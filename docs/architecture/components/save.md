...
- [include/aoc/save/Serializer.hpp](../../../include/aoc/save/Serializer.hpp) —
  ...
  `CURRENT_SAVE_VERSION = 16` (v16: the city record carries the governor component, so
  seated governors and their titles survive a load; v15: the unit record carries the formation level, so a
  ...
  worldgen-only layers live in the headless `.aocmap` cache). Currently 39 named
