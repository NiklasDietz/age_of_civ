...
/// C-string verbatim, so user/Lua-supplied payloads could contain
/// quotes that would break the JSON envelope or, worse, allow
/// attacker-controlled keys/values into the response. Control
/// characters (0x00-0x1F) are illegal raw inside a JSON string, so we
/// emit the standard short escapes for `\n \r \t` and `\u00XX` for the
/// rest -- otherwise a strict parser would reject the whole response.
