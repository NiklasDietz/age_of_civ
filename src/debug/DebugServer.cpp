...
/// Public (declared in the header): also used to escape reflected dump
/// paths. Forwards to the canonical implementation in `aoc::core` so
/// `aoc::ui` (and any other headless-safe or gfx-only consumer) can share
/// one escaper instead of each maintaining its own copy.
std::string escapeJsonString(std::string_view raw) {
    return aoc::core::escapeJsonString(raw);
}
...
bool DebugServer::start() {
    ...
    if (!this->m_impl->server.bind_to_port(kHost, this->m_impl->port)) {
        LOG_WARN("DebugServer: failed to bind to %s:%d (port in use?)", kHost, this->m_impl->port);
        return false;
    }
    ...
    this->m_impl->listener = std::jthread([this]() {
        ...
        try {
            LOG_INFO("DebugServer: listening on http://127.0.0.1:%d", this->m_impl->port);
...
bool DebugServer::isRunning() const noexcept {
    return this->m_impl && this->m_impl->running.load(std::memory_order_acquire);
}
