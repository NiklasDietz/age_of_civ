...
bool processCurrencyCrisis(aoc::game::GameState& gameState, MonetaryStateComponent& state,
                           CurrencyCrisisComponent& crisis, CurrencyTrustComponent& trust) {
    ...
    if (crisis.activeCrisis != CrisisType::None) {
        ...
        case CrisisType::BankRun: {
            // Private specie (backing pool) drains each turn
            const CurrencyAmount drain = std::max<CurrencyAmount>(
                1, static_cast<CurrencyAmount>(static_cast<float>(state.privateSpecie) *
                                               BANK_RUN_DRAIN_RATE));
            state.privateSpecie = std::max<CurrencyAmount>(0, state.privateSpecie - drain);

            // If specie exhausted: forced devaluation
            if (state.privateSpecie <= 0 && state.system == MonetarySystemType::GoldStandard) {
                // Paper currency loses 50% of value
                adjustMoneySupply(state, -state.moneySupply / 2, "bankRunDevaluation");
        ...
        if (crisis.turnsRemaining <= 0) {
            ...
            LOG_INFO("Player %u: %s crisis resolved", static_cast<unsigned>(state.owner),
    ...
    // 2. Bank Run (Gold Standard only) -- triggers on debt/specie ratio
    else if (state.system == MonetarySystemType::GoldStandard && state.privateSpecie > 0 &&
             state.inflationRate > BANK_RUN_INFLATION_THRESHOLD) {
        float debtToGold = (state.privateSpecie > 0) ? static_cast<float>(state.governmentDebt) /
                                                           static_cast<float>(state.privateSpecie)
                                                     : 999.0f;
        if (debtToGold > BANK_RUN_DEBT_RATIO) {
            crisis.activeCrisis   = CrisisType::BankRun;
            crisis.turnsRemaining = BANK_RUN_DURATION;
            newCrisis             = true;
            LOG_INFO("Player %u: BANK RUN! Debt/specie ratio %.1f, inflation %.0f%%",
                     static_cast<unsigned>(state.owner), debtToGold, state.inflationRate * 100.0f);
        }
    }
    // 3. Sovereign Default (any system with debt)
        ...
        if (state.treasury < interestDue && !crisis.hasDefaulted) {
            ...
            LOG_INFO("Player %u: SOVEREIGN DEFAULT! Cannot pay interest %lld, treasury %lld",
                     static_cast<unsigned>(state.owner), static_cast<long long>(interestDue),
...
void executeCurrencyReform(MonetaryStateComponent& state, CurrencyCrisisComponent& crisis,
    ...
    LOG_INFO("Player %u: currency reform executed. Money supply halved, debt halved, borrow "
             "lockout 30t, trust cap 50t.",
             static_cast<unsigned>(state.owner));
}
...
void processReserveStress(MonetaryStateComponent& state, CurrencyTrustComponent& trust) {
    ...
    if (ratio < RESERVE_RUN_THRESHOLD && state.privateSpecie > 0) {
        // Redemption run: foreign holders redeem paper for specie.
        const CurrencyAmount drain = std::max<CurrencyAmount>(
            1, static_cast<CurrencyAmount>(static_cast<float>(state.privateSpecie) *
                                           REDEMPTION_RUN_DRAIN));
        state.privateSpecie = std::max<CurrencyAmount>(0, state.privateSpecie - drain);
        if (!state.redemptionRunActive) {
            state.redemptionRunActive = true;
            LOG_INFO("Player %u: REDEMPTION RUN! Reserve ratio %.2f, %lld specie drained",
                     static_cast<unsigned>(state.owner), static_cast<double>(ratio),
