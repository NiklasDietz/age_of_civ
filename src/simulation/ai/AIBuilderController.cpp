    ...
    for (const BuilderSnapshot& builder : builders) {
        ...
        if (grid.owner(currentIdx) == this->m_player &&
            ...
            if (bestImpr == aoc::map::ImprovementType::None && canChopAt(grid, currentIdx) &&
                ...
                // requestChop retires the builder itself when that was its last
                // charge, so builder.ptr may already be freed: do not touch it,
                // and do not queue it for the trailing removal pass either.
                continue;
            }
            if (bestImpr != aoc::map::ImprovementType::None &&
                ...
                if (!builder.ptr->hasCharges()) {
                    ...
                    LOG_INFO("AI %u Builder exhausted after improving (%d,%d)",
                             static_cast<unsigned>(this->m_player), builder.position.q,
                             builder.position.r);
                    continue;
                }
        ...
        if (gsPlayer->hasResearched(TechId{24}) && grid.owner(currentIdx) == this->m_player &&
            grid.improvement(currentIdx) == aoc::map::ImprovementType::None &&
            !grid.resource(currentIdx).isValid() && (currentIdx % 8 == 3) &&
            canPlaceImprovement(grid, currentIdx, aoc::map::ImprovementType::Greenhouse)) {
            grid.setImprovement(currentIdx, aoc::map::ImprovementType::Greenhouse);
            builder.ptr->useCharge();
            LOG_INFO("AI %u Builder built Greenhouse at (%d,%d)",
                     static_cast<unsigned>(this->m_player), builder.position.q, builder.position.r);
            if (!builder.ptr->hasCharges()) {
                exhaustedBuilders.push_back(builder.ptr);
            }
            continue;
        ...
        if (grid.improvement(currentIdx) == aoc::map::ImprovementType::Greenhouse &&
            ...
            if (bestGood != 0xFFFFu && sourceCity != nullptr) {
                if (aoc::sim::plantGreenhouseCrop(grid, sourceCity->stockpile(), currentIdx,
                                                  bestGood)) {
                    builder.ptr->useCharge();
        ...
        if (gsPlayer->hasResearched(TechId{15}) && grid.owner(currentIdx) == this->m_player &&
            ...
            if (res.isValid() &&
                (res.value == aoc::sim::goods::OIL || res.value == aoc::sim::goods::NATURAL_GAS)) {
            ...
            if (passableLand) {
                ...
                if (closeOwn <= 12 && closeOwn >= 2 && existingPosts < 8 + this->m_player) {
                    if (canPlaceImprovement(grid, currentIdx,
                        ...
                        grid.setImprovement(currentIdx, aoc::map::ImprovementType::TradingPost);
        ...
        if (gsPlayer->hasResearched(TechId{6}) && grid.owner(currentIdx) == this->m_player &&
            grid.improvement(currentIdx) == aoc::map::ImprovementType::None) {
            ...
            if (nearBorder) {
                ...
                if (!tooClose &&
                    canPlaceImprovement(grid, currentIdx, aoc::map::ImprovementType::Encampment)) {
                    grid.setImprovement(currentIdx, aoc::map::ImprovementType::Encampment);
        ...
        for (const aoc::hex::AxialCoord& cityLoc : cityLocations) {
            ...
            for (const aoc::hex::AxialCoord& tile : searchTiles) {
                ...
                if (grid.owner(tIdx) != this->m_player) {
                    continue;
                }
                if (grid.movementCost(tIdx) <= 0) {
                    continue;
                }
            ...
            if (existingPosts < postCap) {
                ...
                for (const aoc::hex::AxialCoord& fc : foreignCities) {
                    for (const aoc::hex::AxialCoord& cityLoc : cityLocations) {
                        ...
                        if (pairDist <= 5 || pairDist > 40) {
                            continue;
                        }
                        const int32_t midSteps = pairDist / 2;
                        const float lerp =
                            static_cast<float>(midSteps) / static_cast<float>(pairDist);
                        const int32_t mq = static_cast<int32_t>(
                            std::round(static_cast<float>(cityLoc.q) * (1.0f - lerp) +
                                       static_cast<float>(fc.q) * lerp));
                        const int32_t mr = static_cast<int32_t>(
                            std::round(static_cast<float>(cityLoc.r) * (1.0f - lerp) +
                        ...
                        // razed-city tile etc.).
                        if (grid.improvement(midIdx) == aoc::map::ImprovementType::TradingPost) {
                            continue;
                        }
                        if (grid.movementCost(midIdx) <= 0) {
                            continue;
                        }
                        // Accept any passable LAND tile (not water, not mountain).
                        const aoc::map::TerrainType tt = grid.terrain(midIdx);
                        if (aoc::map::isWater(tt)) {
                            continue;
                        }
                        if (tt == aoc::map::TerrainType::Mountain) {
                            continue;
                        }
                        if (targetedTiles.find(mid) != targetedTiles.end()) {
                            continue;
                        }
