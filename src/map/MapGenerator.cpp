...
void MapGenerator::assignTerrain(const Config& config, HexGrid& grid, aoc::Random& rng) {
    ...
    if (config.mapType == MapType::Continents) {
        ...
        // ============================================================
        // SESSION 17 — PET / aridity / erosion potential / carbon stock
        // / wilderness / flood frequency / canopy stratification /
        // riparian forest / magnetic intensity / groundwater depth.
        // ============================================================
        ...
        for (int32_t row = 0; row < height; ++row) {
            ...
            for (int32_t col = 0; col < width; ++col) {
                ...
                if (t != aoc::map::TerrainType::Ocean
                    ...
                    // K-factor by soil texture (silty soils most erodible).
                    const auto& siltP = grid.soilSiltPct();
                    if (siltP.size() > si) {
                        E += static_cast<int32_t>(siltP[si]) / 4;
                    }
                    // Vegetation cover protects.
                    const auto& vd = grid.vegetationDensity();
                    if (vd.size() > si) {
                ...
                // ---- CARBON STOCK ----
                int32_t C = 30;
                if (f == aoc::map::FeatureType::Jungle) { C = 220; }
                else if (f == aoc::map::FeatureType::Forest) {
                    C = (lat < 0.20f) ? 200 : (lat > 0.55f ? 180 : 150);
                }
                else if (f == aoc::map::FeatureType::Marsh) { C = 180; }
                else if (f == aoc::map::FeatureType::Floodplains) { C = 120; }
                else if (t == aoc::map::TerrainType::Grassland) { C = 90; }
                else if (t == aoc::map::TerrainType::Plains) { C = 60; }
                // Soil carbon boost from histosols.
                const auto& so = grid.soilOrder();
                ...
                // ---- WILDERNESS ----
                // High NPP + high biodiversity + isolated.
                ...
                if (f == aoc::map::FeatureType::Jungle) {
                    canopy[si] = 4; // emergent + canopy + understory + floor
                } else if (f == aoc::map::FeatureType::Forest) {
                    if (lat < 0.30f) {
                        canopy[si] = 3; // tropical-ish
                    } else if (lat > 0.55f) {
                        canopy[si] = 2; // boreal simple
                    } else {
                        canopy[si] = 3; // temperate canopy + understory
                    }
                } else if ((t == aoc::map::TerrainType::Plains
                     || t == aoc::map::TerrainType::Grassland)
                    && lat < 0.30f
                    && f == aoc::map::FeatureType::None) {
                    canopy[si] = 1; // savanna scrub
                }
                ...
                if (grid.riverEdges(i) != 0) {
                    int32_t r = 80;
                    if (t == aoc::map::TerrainType::Desert) {
                        r = 200; // gallery forest stark contrast
                    } else if (f == aoc::map::FeatureType::Jungle
                            || f == aoc::map::FeatureType::Forest) {
                        r = 150;
                    } else if (lat > 0.55f) {
                        r = 100;
                ...
                // ---- MAGNETIC INTENSITY ----
                // Vertical at poles → high intensity. Horizontal at
                // equator → low intensity. Earth ~25-65 μT.
                const float magI_K = 90.0f + lat * 165.0f;
                magI[si] = static_cast<uint8_t>(
                    std::clamp(magI_K, 0.0f, 255.0f));

                // ---- GROUNDWATER DEPTH ----
                // Wetlands/floodplains = at-surface (low value).
                // Deserts deep. Riparian shallow.
                int32_t depth = 130;
                if (f == aoc::map::FeatureType::Marsh
                    || f == aoc::map::FeatureType::Floodplains) {
                    depth = 5;
...
// BoundaryType lives in aoc/map/gen/PlateBoundary.hpp so the resource
// placer pass (now in src/map/gen/Resources.cpp) can share the enum.
