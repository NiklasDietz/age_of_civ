...
const char* wikiCategoryName(WikiCategory cat) {
    switch (cat) {
    case WikiCategory::Units:
        return "Units";
    case WikiCategory::Buildings:
        return "Buildings";
    case WikiCategory::Improvements:
        return "Improvements";
    case WikiCategory::Resources:
        return "Resources & Goods";
    case WikiCategory::Goods:
        return "Goods";
    case WikiCategory::Recipes:
        return "Recipes";
    case WikiCategory::Technologies:
        return "Technologies";
    case WikiCategory::Governments:
        return "Governments";
    case WikiCategory::Policies:
        return "Policy Cards";
    case WikiCategory::Wonders:
        return "Wonders";
    case WikiCategory::Civilizations:
        return "Civilizations";
    case WikiCategory::Mechanics:
...
static std::string yieldStr(const aoc::map::TileYield& y) {
    std::string s;
    if (y.food > 0) {
        s += "Food:" + std::to_string(y.food) + " ";
    }
    if (y.production > 0) {
        s += "Prod:" + std::to_string(y.production) + " ";
    }
    if (y.gold > 0) {
        s += "Gold:" + std::to_string(y.gold) + " ";
    }
    if (y.science > 0) {
        s += "Sci:" + std::to_string(y.science) + " ";
    }
    if (y.culture > 0) {
        s += "Cul:" + std::to_string(y.culture) + " ";
    }
    if (y.faith > 0) {
        s += "Faith:" + std::to_string(y.faith) + " ";
    }
    if (s.empty()) {
        s = "None";
    }
...
static void buildUnitEntries(std::vector<WikiEntry>& entries) {
    for (const aoc::sim::UnitTypeDef& u : aoc::sim::UNIT_TYPE_DEFS) {
        ...
        entry.statsBlock =
            "Class: " + unitClassName(u.unitClass) + "\nHP: " + std::to_string(u.maxHitPoints) +
...
static void buildBuildingEntries(std::vector<WikiEntry>& entries) {
    for (const aoc::sim::BuildingDef& b : aoc::sim::BUILDING_DEFS) {
        ...
        entry.statsBlock =
            "District: " + std::string(aoc::sim::districtTypeName(b.requiredDistrict)) +
        ...
        if (b.productionBonus > 0) {
            bonuses += "+Prod:" + std::to_string(b.productionBonus) + " ";
        }
        if (b.scienceBonus > 0) {
            bonuses += "+Sci:" + std::to_string(b.scienceBonus) + " ";
        }
        if (b.goldBonus > 0) {
            bonuses += "+Gold:" + std::to_string(b.goldBonus) + " ";
        }
        ...
        if (b.requiredCivic.isValid()) {
            entry.statsBlock +=
                "\nRequires Civic: " + std::string(aoc::sim::civicDef(b.requiredCivic).name);
        }

        // Building capacity info
        aoc::sim::BuildingTierClass tier = aoc::sim::buildingTierClass(b.id);
        entry.body =
            "Throughput capacity: Lv1=" +
        ...
        for (const aoc::sim::PowerPlantDef& pp : aoc::sim::POWER_PLANT_DEFS) {
            if (pp.buildingId == b.id) {
                ...
                if (pp.requiresRiver) {
                    entry.body += ", requires river";
                }
                if (pp.meltdownRisk > 0.0f) {
                    entry.body += ", MELTDOWN RISK 0.2%/turn";
                }
                break;
            }
        }
...
static void buildImprovementEntries(std::vector<WikiEntry>& entries) {
    for (const aoc::sim::ImprovementDef& imp : aoc::sim::IMPROVEMENT_DEFS) {
        if (imp.type == aoc::map::ImprovementType::None) {
            continue;
        }
        WikiEntry entry;
...
static void buildGoodEntries(std::vector<WikiEntry>& entries) {
    ...
    for (uint16_t i = 0; i < count; ++i) {
        ...
        case aoc::sim::GoodCategory::RawBonus:
            catName = "Raw Bonus";
            break;
        case aoc::sim::GoodCategory::Processed:
            catName = "Processed";
            break;
        case aoc::sim::GoodCategory::Advanced:
            catName = "Advanced";
            break;
        // GoodCategory::Monetary removed in Phase B
        default:
            break;
        }
        ...
        case aoc::sim::ClimateBand::Tropical:
            bandName = "Tropical";
            break;
        case aoc::sim::ClimateBand::Subtropical:
            bandName = "Subtropical";
            break;
        case aoc::sim::ClimateBand::Temperate:
            bandName = "Temperate";
            break;
        case aoc::sim::ClimateBand::Cold:
            bandName = "Cold";
            break;
        default:
            break;
...
static void buildRecipeEntries(std::vector<WikiEntry>& entries) {
    for (const aoc::sim::ProductionRecipe& r : aoc::sim::allRecipes()) {
        ...
        for (const aoc::sim::RecipeInput& in : r.inputs) {
            inputs += std::string(aoc::sim::goodDef(in.goodId).name) + " x" +
                      std::to_string(in.amount) + (in.consumed ? "" : " (catalyst)") + ", ";
        }
        if (!inputs.empty()) {
        ...
        entry.statsBlock = "Inputs: " + inputs +
                           ...
                           std::string(aoc::sim::buildingDef(r.requiredBuilding).name) +
                           "\nProcess Time: " + std::to_string(r.turnsToProcess) + " turn(s)";

        entry.body =
            "Production recipe. City must have the required building and"
...
static void buildTechEntries(std::vector<WikiEntry>& entries) {
    ...
    for (const aoc::sim::TechDef& t : techs) {
        ...
        entry.statsBlock = "Era: " + std::to_string(t.era.value) +
                           "\nBase Research Cost: " + std::to_string(t.researchCost) +
                           " science (scaled by pace and tree depth)";
        ...
        if (entry.body == "Unlocks:") {
            entry.body = "No direct unlocks listed.";
        }
...
static void buildGovernmentEntries(std::vector<WikiEntry>& entries) {
    for (const aoc::sim::GovernmentDef& g : aoc::sim::GOVERNMENT_DEFS) {
        ...
        entry.statsBlock =
            "Slots: M:" + std::to_string(g.militarySlots) +
            " E:" + std::to_string(g.economicSlots) + " D:" + std::to_string(g.diplomaticSlots) +
            " W:" + std::to_string(g.wildcardSlots) +
            "\nCorruption Rate: " + std::to_string(static_cast<int>(g.corruptionRate * 100.0f)) +
            "% per excess city";
        ...
        if (g.inherentBonuses.productionMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>(
                              (g.inherentBonuses.productionMultiplier - 1.0f) * 100.0f)) +
                          "% Production";
        }
        if (g.inherentBonuses.goldMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>(
                              (g.inherentBonuses.goldMultiplier - 1.0f) * 100.0f)) +
                          "% Gold";
        }
        if (g.inherentBonuses.scienceMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>(
                              (g.inherentBonuses.scienceMultiplier - 1.0f) * 100.0f)) +
                          "% Science";
        }
        if (g.inherentBonuses.cultureMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>(
                              (g.inherentBonuses.cultureMultiplier - 1.0f) * 100.0f)) +
                          "% Culture";
        }
        if (g.inherentBonuses.combatStrengthBonus > 0.5f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>(g.inherentBonuses.combatStrengthBonus)) +
                          " Combat Strength";
        }
        ...
        if (action != aoc::sim::GovernmentAction::None) {
            ...
            case aoc::sim::GovernmentAction::Referendum:
                actionName = "Referendum";
                actionDesc = "+20 loyalty all cities for 5 turns";
                break;
            case aoc::sim::GovernmentAction::FiveYearPlan:
                actionName = "Five Year Plan";
                actionDesc = "+30% production for 10 turns, -2 amenities";
                break;
            case aoc::sim::GovernmentAction::Mobilization:
                actionName = "Mobilization";
                actionDesc = "Instantly conscript 3 military units at capital";
                break;
            case aoc::sim::GovernmentAction::RoyalDecree:
                actionName = "Royal Decree";
                actionDesc = "+15% gold for 10 turns";
                break;
            case aoc::sim::GovernmentAction::HolyWar:
                actionName = "Holy War";
                actionDesc = "+4 combat, +20% faith for 10 turns";
                break;
            case aoc::sim::GovernmentAction::TradeFleet:
                actionName = "Trade Fleet";
                actionDesc = "+3 trade routes for 10 turns";
                break;
            default:
                break;
            }
        ...
        entry.body += "\n\nChanging to this government causes " +
                      std::to_string(aoc::sim::ANARCHY_DURATION) + " turns of anarchy.";
...
static void buildPolicyEntries(std::vector<WikiEntry>& entries) {
    for (const aoc::sim::PolicyCardDef& p : aoc::sim::POLICY_CARD_DEFS) {
        ...
        switch (p.slotType) {
        case aoc::sim::PolicySlotType::Military:
            slotName = "Military";
            break;
        case aoc::sim::PolicySlotType::Economic:
            slotName = "Economic";
            break;
        case aoc::sim::PolicySlotType::Diplomatic:
            slotName = "Diplomatic";
            break;
        case aoc::sim::PolicySlotType::Wildcard:
            slotName = "Wildcard";
            break;
        }
        ...
        if (m.productionMultiplier > 1.01f) {
            entry.body +=
                "\n  +" +
                std::to_string(static_cast<int>((m.productionMultiplier - 1.0f) * 100.0f)) +
                "% Production";
        }
        if (m.goldMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>((m.goldMultiplier - 1.0f) * 100.0f)) +
                          "% Gold";
        }
        if (m.scienceMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>((m.scienceMultiplier - 1.0f) * 100.0f)) +
                          "% Science";
        }
        if (m.cultureMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>((m.cultureMultiplier - 1.0f) * 100.0f)) +
                          "% Culture";
        }
        if (m.combatStrengthBonus > 0.5f) {
            entry.body += "\n  +" + std::to_string(static_cast<int>(m.combatStrengthBonus)) +
                          " Combat Strength";
        }
        if (m.tradeRouteBonus > 0.01f) {
            entry.body += "\n  +" + std::to_string(static_cast<int>(m.tradeRouteBonus)) +
                          " Gold per trade route";
        }
        if (m.unitMaintenanceReduction > 0.01f) {
            entry.body += "\n  -" + std::to_string(static_cast<int>(m.unitMaintenanceReduction)) +
                          " Unit maintenance";
        }
        if (m.productionPerCity > 0.01f) {
            entry.body += "\n  +" + std::to_string(static_cast<int>(m.productionPerCity)) +
                          " Production per city";
        }
        if (m.extraTradeRoutes > 0) {
            entry.body += "\n  +" + std::to_string(m.extraTradeRoutes) + " Trade routes";
        }
        if (m.loyaltyBonus > 0.5f) {
            entry.body += "\n  +" + std::to_string(static_cast<int>(m.loyaltyBonus)) + " Loyalty";
        }
        if (m.warWearinessReduction < -0.01f) {
            entry.body += "\n  " +
                          std::to_string(static_cast<int>(m.warWearinessReduction * 100.0f)) +
                          "% War Weariness";
        }
        if (m.faithMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>((m.faithMultiplier - 1.0f) * 100.0f)) +
                          "% Faith";
        }
        if (m.corruptionReduction > 0.001f) {
            entry.body += "\n  -" +
                          std::to_string(static_cast<int>(m.corruptionReduction * 100.0f)) +
                          "% Corruption";
        }
        if (m.espionageDefense > 0.01f) {
            entry.body += "\n  +" + std::to_string(static_cast<int>(m.espionageDefense * 100.0f)) +
                          "% Espionage Defense";
        }
        if (m.tariffEfficiency > 0.01f) {
            entry.body += "\n  +" + std::to_string(static_cast<int>(m.tariffEfficiency * 100.0f)) +
                          "% Tariff Efficiency";
        }
        if (m.growthMultiplier > 1.01f) {
            entry.body += "\n  +" +
                          std::to_string(static_cast<int>((m.growthMultiplier - 1.0f) * 100.0f)) +
                          "% Population Growth";
        }
...
static void buildMechanicsEntries(std::vector<WikiEntry>& entries) {
    // WP-C3 Power Grid
    entries.push_back(
        {WikiCategory::Mechanics, "Power Grid",
    ...
    // Monetary System
    entries.push_back(
        {WikiCategory::Mechanics, "Monetary System",
         "Your civilization's money evolves through 4 stages:\n\n"
         "1. BARTER: Direct good-for-good exchange. Limited trade.\n"
         "2. COMMODITY MONEY: Metal coins (copper/silver/gold). Trade efficiency depends on coin "
         "tier.\n"
    ...
    // Currency Crises
    entries.push_back(
        {WikiCategory::Mechanics, "Currency Crises",
    ...
    // Trade
    entries.push_back(
        {WikiCategory::Mechanics, "Trade System",
    ...
    // Government
    entries.push_back(
        {WikiCategory::Mechanics, "Government System",
...
void EncyclopediaScreen::open(UIManager& ui) {
    ...
    WidgetId innerPanel       = this->createScreenFrame(ui, "Encyclopedia", PANEL_W, PANEL_H,
    ...
    this->m_categoryPanel = ui.createPanel(innerPanel, {0.0f, 0.0f, CONTENT_W, 52.0f},
    ...
    this->m_detailLabel = ui.createLabel(
        body, {0.0f, 0.0f, CONTENT_W - 240.0f, 400.0f},
