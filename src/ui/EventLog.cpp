...
void EventLog::render(vulkan_app::renderer::Renderer2D& renderer2d, float x, float y, float w,
                      float h, float pixelScale) const {
    if (this->m_events.empty()) {
        return;
    }

    // Draw semi-transparent background panel
    renderer2d.drawFilledRect(x, y, w, h, tokens::SURFACE_INK.r, tokens::SURFACE_INK.g,
    ...
    const std::size_t startIdx =
        (this->m_events.size() > linesToShow) ? this->m_events.size() - linesToShow : 0;

    float lineY = y + PADDING;
    for (std::size_t i = startIdx; i < this->m_events.size(); ++i) {
        BitmapFont::drawText(renderer2d, this->m_events[i], x + PADDING, lineY, FONT_SIZE,
                             tokens::TEXT_PARCHMENT, pixelScale);
        lineY += LINE_HEIGHT;
    }
}
