...
void UIManager::selectOnly(WidgetId id) {
    ...
    for (WidgetId sib :
         (parent != INVALID_WIDGET) ? this->m_widgets[parent].children : this->m_rootWidgets) {
        Widget* sw = this->getWidget(sib);
        if (sw != nullptr && sw->selectable) {
            sw->isSelected = false;
        }
    }
    w->isSelected        = true;
...
void UIManager::selectRangeTo(WidgetId id) {
    ...
    const std::vector<WidgetId>& siblings =
        (w->parent != INVALID_WIDGET) ? this->m_widgets[w->parent].children : this->m_rootWidgets;
...
std::vector<WidgetId> UIManager::currentSelection(WidgetId parent) const {
    ...
    const std::vector<WidgetId>& siblings =
        (parent != INVALID_WIDGET && parent < this->m_widgets.size())
            ? this->m_widgets[parent].children
            : this->m_rootWidgets;
    for (WidgetId sib : siblings) {
        const Widget* sw = this->getWidget(sib);
        if (sw != nullptr && sw->selectable && sw->isSelected) {
            out.push_back(sib);
        }
    }
    return out;
...
bool UIManager::isLive(WidgetHandle handle) const {
    if (handle.id == INVALID_WIDGET) {
        return false;
    }
    if (handle.id >= this->m_widgets.size()) {
        return false;
    }
    if (this->m_widgets[handle.id].id == INVALID_WIDGET) {
        return false;
    }
    const uint32_t gen =
        (handle.id < this->m_generations.size()) ? this->m_generations[handle.id] : 0;
...
WidgetId UIManager::focusNext() {
    ...
    WidgetId next =
        (it == list.end() || std::next(it) == list.end()) ? list.front() : *std::next(it);
...
std::string UIManager::dumpTreeJson() const {
    ...
    for (const Widget& w : this->m_widgets) {
        ...
        if (std::holds_alternative<ButtonData>(w.data)) {
            ...
            kind = "scroll";
        } else if (std::holds_alternative<TabBarData>(w.data)) {
            kind = "tabs";
        } else if (std::holds_alternative<ProgressBarData>(w.data)) {
            kind = "progress";
        } else if (std::holds_alternative<SliderData>(w.data)) {
            kind = "slider";
        ...
        if (pressed != nullptr) {
            if (SliderData* slider = std::get_if<SliderData>(&pressed->data)) {
                ...
                if (maxX > minX) {
                    ...
                    float v = slider->minValue + t * (slider->maxValue - slider->minValue);
                    if (slider->step > 0.0f) {
                        v = slider->minValue +
                            std::round((v - slider->minValue) / slider->step) * slider->step;
                    }
                    if (v != slider->value) {
                        slider->value = v;
    ...
    if (hit != INVALID_WIDGET) {
        ...
        if (scrollDelta != 0.0f) {
            ...
            // with parent ScrollList/canvas pan logic.
            {
                ...
                if (hitW != nullptr && std::holds_alternative<ButtonData>(hitW->data)) {
        ...
        if (rightReleased) {
            ...
            if (this->m_rightPressedWidget != INVALID_WIDGET && this->m_rightPressedWidget != hit) {
...
WidgetId UIManager::hitTest(float x, float y) const {
    ...
    for (std::vector<WidgetId>::const_reverse_iterator it = this->m_rootWidgets.rbegin();
         it != this->m_rootWidgets.rend(); ++it) {
...
WidgetId UIManager::hitTestWidget(WidgetId id, float x, float y) const {
    ...
    for (std::vector<WidgetId>::const_reverse_iterator it = w->children.rbegin();
         it != w->children.rend(); ++it) {
        WidgetId result = this->hitTestWidget(*it, x, y);
        if (result != INVALID_WIDGET) {
            return result;
        }
    ...
    if (std::holds_alternative<ButtonData>(w->data) || std::holds_alternative<PanelData>(w->data) ||
        std::holds_alternative<ScrollListData>(w->data) ||
        std::holds_alternative<TabBarData>(w->data) ||
        std::holds_alternative<SliderData>(w->data) || std::holds_alternative<IconData>(w->data) ||
...
void UIManager::layoutWidget(WidgetId id, float parentX, float parentY) {
    ...
    const float axisTotal = (w->layoutDirection == LayoutDirection::Vertical) ? contentH : contentW;
    const float spacingTotal =
        (visibleCount > 1) ? static_cast<float>(visibleCount - 1) * w->childSpacing : 0.0f;
    const float flexPool =
        (flexSum > 0.0f) ? std::max(0.0f, axisTotal - intrinsicTotal - spacingTotal) : 0.0f;
        ...
        if (w->layoutDirection == LayoutDirection::None) {
            ...
            if (w->clampChildren) {
                const float overflow =
                    (child->computedBounds.x + child->computedBounds.w) - (contentX + contentW);
                if (overflow > 0.0f) {
            ...
            if (cursorX > contentX && (cursorX - contentX) + childW > contentW) {
                cursorX = contentX;
    ...
    if (ScrollListData* scrollData = std::get_if<ScrollListData>(&w->data)) {
        float totalHeight = (cursorY - scrollOffsetY) - (w->computedBounds.y + w->padding.top);
        if (!w->children.empty()) {
...
void UIManager::renderWidget(vulkan_app::renderer::Renderer2D& renderer2d, WidgetId id) const {
    const Widget* w = this->getWidget(id);
    if (w == nullptr || !w->isVisible) {
        return;
    }
        ...
        [&](const auto& data) {
            ...
            if constexpr (std::is_same_v<T, PanelData>) {
                ...
                if (data.gradientBottom.a > 0.0f) {
                    renderer2d.drawGradientRect(
                        b.x, b.y, b.w, b.h, data.backgroundColor.r, data.backgroundColor.g,
                        data.backgroundColor.b, data.backgroundColor.a * w->alpha,
                        data.gradientBottom.r, data.gradientBottom.g, data.gradientBottom.b,
                        data.gradientBottom.a * w->alpha, false);
                }
                // Border.
                if (data.borderColor.a > 0.0f) {
                    const float bw = std::max(1.0f, data.borderWidth * scale);
                    const float ba = data.borderColor.a * w->alpha;
                    renderer2d.drawRect(b.x, b.y, b.w, b.h, data.borderColor.r, data.borderColor.g,
                                        data.borderColor.b, ba, bw);
                }
                // Top highlight + bottom shadow: cheap depth cues.
                if (data.topHighlight.a > 0.0f) {
                    renderer2d.drawFilledRect(b.x, b.y, b.w, 1.0f * scale, data.topHighlight.r,
                                              data.topHighlight.g, data.topHighlight.b,
                                              data.topHighlight.a * w->alpha);
                }
                ...
                float cr = data.cornerRadius * scale;
                if (cr > 0.0f) {
                    renderer2d.drawRoundedRect(b.x, b.y, b.w, b.h, cr, color.r, color.g, color.b,
                                               color.a);
                } else {
                    renderer2d.drawFilledRect(b.x, b.y, b.w, b.h, color.r, color.g, color.b,
                                              color.a);
                }

                // Gradient bottom band for glossy depth. Smooth GPU gradient
                // from the active state colour to gradientBottom. Off when alpha 0.
                if (data.gradientBottom.a > 0.0f) {
                    renderer2d.drawGradientRect(b.x, b.y, b.w, b.h, color.r, color.g, color.b,
                                                color.a, data.gradientBottom.r,
                                                data.gradientBottom.g, data.gradientBottom.b,
                                                data.gradientBottom.a, false);
                }
                ...
                if (data.borderColor.a > 0.0f) {
                    const float bw = std::max(1.0f, data.borderWidth * scale);
                    if (cr > 0.0f) {
                        renderer2d.drawRoundedRect(b.x, b.y, b.w, b.h, cr, data.borderColor.r,
                                                   data.borderColor.g, data.borderColor.b,
                                                   data.borderColor.a, bw);
                    } else {
                        renderer2d.drawRect(b.x, b.y, b.w, b.h, data.borderColor.r,
                                            data.borderColor.g, data.borderColor.b,
                                            data.borderColor.a, bw);
                    }
                }
                ...
                if (!data.disabled) {
                    ...
                    if (w->isPressed) {
                        ...
                        renderer2d.drawFilledRect(b.x, b.y, b.w, edge, 0.0f, 0.0f, 0.0f, 0.35f);
                        renderer2d.drawFilledRect(b.x, b.y, edge, b.h, 0.0f, 0.0f, 0.0f, 0.35f);
                    } else {
                        // raised: brighten top
                        renderer2d.drawFilledRect(b.x, b.y, b.w, edge, 1.0f, 1.0f, 1.0f, 0.15f);
                    }
                }
                ...
                if (w->isFocused) {
                    ...
                    renderer2d.drawFilledRect(b.x, b.y, b.w, ring, 1.0f, 0.9f, 0.3f, 0.9f);
                    renderer2d.drawFilledRect(b.x, b.y + b.h - ring, b.w, ring, 1.0f, 0.9f, 0.3f,
                                              0.9f);
                    renderer2d.drawFilledRect(b.x, b.y, ring, b.h, 1.0f, 0.9f, 0.3f, 0.9f);
                    renderer2d.drawFilledRect(b.x + b.w - ring, b.y, ring, b.h, 1.0f, 0.9f, 0.3f,
                                              0.9f);
                }
                ...
                if (data.iconSpriteId != 0) {
                    ...
                    renderer2d.drawFilledRect(b.x + 4.0f * scale, b.y + (b.h - iz) * 0.5f, iz, iz,
                                              ic.r, ic.g, ic.b, ic.a);
                    labelOffset = iz + 6.0f * scale;
                }
                // Center the label text in the remaining area. Pass pixelScale so each rasterized
                // pixel is scale x scale world units, which the shader zooms back to 1x1 screen
                // pixels.
                if (!data.label.empty()) {
                    ...
                    float textX         = b.x + labelOffset + (b.w - labelOffset - textW) * 0.5f;
                ...
                for (std::size_t i = 0; i < data.labels.size(); ++i) {
                    ...
                    if (w->isHovered && !isActive) {
                        c = data.hoverColor;
                    }
                    renderer2d.drawFilledRect(tx, b.y, tabW, b.h, c.r, c.g, c.b, c.a);
                    ...
                    BitmapFont::drawText(renderer2d, data.labels[i], textX, textY, worldFontSize,
                                         data.labelColor, scale);
                }
                ...
                if (fillW > 0.0f) {
                    if (cr > 0.0f) {
                        renderer2d.drawRoundedRect(b.x, b.y, fillW, b.h, cr, data.fillColor.r,
                                                   data.fillColor.g, data.fillColor.b,
                                                   data.fillColor.a);
                    } else {
                        renderer2d.drawFilledRect(b.x, b.y, fillW, b.h, data.fillColor.r,
                                                  data.fillColor.g, data.fillColor.b,
                                                  data.fillColor.a);
                    }
                }
                if (!data.overlayText.empty()) {
                    ...
                    const float textW = tb.w * scale;
                    const float textH = tb.h * scale;
                    BitmapFont::drawText(renderer2d, data.overlayText, b.x + (b.w - textW) * 0.5f,
                ...
                renderer2d.drawFilledRect(b.x, b.y + b.h * 0.4f, b.w * t, b.h * 0.2f,
                                          data.fillColor.r, data.fillColor.g, data.fillColor.b,
                                          data.fillColor.a);
                // Thumb.
                ...
                for (const RichTextSpan& span : data.spans) {
                    ...
                    if (span.kind == RichTextSpan::Kind::Icon) {
                        ...
                        renderer2d.drawFilledRect(cursorX, cursorY, iw, iw, span.color.r,
                    ...
                    if (!span.text.empty()) {
                        ...
                        if (data.wrapWidth > 0.0f && (cursorX - b.x) + tw > data.wrapWidth) {
                            cursorX = b.x;
                            cursorY += lineHeight;
                        }
                        BitmapFont::drawText(renderer2d, span.text, cursorX, cursorY, worldFontSize,
                                             span.color, scale);
                ...
                renderer2d.drawFilledRect(b.x, b.y, b.w, b.h * 0.6f, c.r * data.tint.r,
                                          c.g * data.tint.g, c.b * data.tint.b, data.tint.a);
                // Title strip.
                const float worldTitle = data.titleFontSize * scale;
                BitmapFont::drawText(renderer2d, data.title, b.x + 6.0f * scale, b.y + b.h * 0.62f,
                ...
                for (const std::pair<std::string, std::string>& kv : data.stats) {
                    BitmapFont::drawText(renderer2d, kv.first, b.x + 6.0f * scale, sy, worldStat,
                                         Color{0.7f, 0.7f, 0.7f, 1.0f}, scale);
                    BitmapFont::drawText(renderer2d, kv.second, b.x + b.w * 0.55f, sy, worldStat,
                ...
                if (bg.a > 0.0f) {
                    renderer2d.drawFilledRect(b.x, b.y, b.w, b.h, bg.r, bg.g, bg.b, bg.a);
                }
                if (w->isSelected) {
                    // Accent edge on the left, 4px wide.
                    const float accW = 4.0f * scale;
                    renderer2d.drawFilledRect(b.x, b.y, accW, b.h, data.accentColor.r,
                                              data.accentColor.g, data.accentColor.b,
                                              data.accentColor.a);
                }
                ...
                if (data.iconSpriteId != 0) {
                    ...
                    const float iz = data.iconSize * scale;
                    renderer2d.drawFilledRect(cursorX, b.y + (b.h - iz) * 0.5f, iz, iz, ic.r, ic.g,
                                              ic.b, ic.a);
                ...
                if (!data.subtitle.empty()) {
                    BitmapFont::drawText(renderer2d, data.title, cursorX, b.y + 4.0f * scale,
                                         titleFont, data.titleColor, scale);
                    BitmapFont::drawText(renderer2d, data.subtitle, cursorX, b.y + b.h * 0.55f,
                                         subFont, data.subtitleColor, scale);
                } else {
                    BitmapFont::drawText(renderer2d, data.title, cursorX,
                                         b.y + (b.h - titleFont) * 0.5f, titleFont, data.titleColor,
                                         scale);
                }
                ...
                for (std::size_t i = 0; i <= src.size(); ++i) {
                    if (i == src.size() || src[i] == '\n') {
                        ...
                        if (!line.empty()) {
                            BitmapFont::drawText(renderer2d, line, b.x, cursorY, font, color,
                                                 scale);
                        }
                        cursorY += lineH;
                        start = i + 1;
