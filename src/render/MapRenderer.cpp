...
void MapRenderer::draw(vulkan_app::renderer::Renderer2D& renderer2d, const aoc::map::HexGrid& grid,
                       const aoc::map::FogOfWar& fog, PlayerId viewingPlayer,
                       const CameraController& camera, uint32_t screenWidth,
    ...
    const int32_t minCol =
        cylindrical ? (static_cast<int32_t>(topLeftX / xSpacing) - 1)
    ...
    const int32_t minRow =
        std::max(0, static_cast<int32_t>(std::max(0.0f, topLeftY) / ySpacing) - 1);
    const int32_t maxRow =
        std::min(height - 1, static_cast<int32_t>(std::max(0.0f, botRightY) / ySpacing) + 1);
...
void MapRenderer::drawTile(vulkan_app::renderer::Renderer2D& renderer2d,
                           const aoc::map::HexGrid& grid, int32_t tileIndex, float cx, float cy,
                           bool dimmed) const {
    ...
    const bool isWater =
        (terrain == map::TerrainType::Ocean || terrain == map::TerrainType::Coast ||
         terrain == map::TerrainType::ShallowWater);

    // Per-tile colour variation: deterministic noise based on tile index.
    // Breaks up otherwise flat terrain without affecting gameplay.
    //
    // Water gets far less of it. Open ocean is a large uniform expanse, so
    // per-hex shade variation there does not read as texture -- it reads as a
    // honeycomb, redrawing the very tile grid the outline pass below skips.
    // Land keeps the full amount, where neighbouring colours differ anyway.
    {
        const float amplitude = isWater ? 0.014f : 0.08f;
        uint32_t noiseHash    = static_cast<uint32_t>(tileIndex) * 2654435761u;
        float noise = (static_cast<float>(noiseHash % 1000u) / 1000.0f - 0.5f) * amplitude;
    ...
    //
    // Water is deliberately excluded. Open ocean is a single continuous
    // surface and often most of the screen, so outlining every cell turned the
    // sea into a honeycomb and added visual noise where there is nothing to
    // discriminate. Land keeps a faint outline because tile boundaries there
    // carry real gameplay meaning (movement, workable tiles, borders).
    if (!dimmed && !isWater) {
        constexpr float BORDER_ALPHA = 0.10f;
        renderer2d.drawHexagonOutline(cx, cy, hexW * 0.98f, hexH * 0.98f, 0.0f, 0.0f, 0.0f,
                                      BORDER_ALPHA, 1.0f);
    }
    ...
    // Rock and snow tones track terrainColor(Mountain) / terrainColor(Snow) --
    // these used to be brighter literals tuned against the pre-gamma-fix
    // output and now read as a separate, lighter material than the tile itself.
    if (terrain == map::TerrainType::Mountain) {
        const float s = hs * 0.38f;
        // Left peak (in shadow)
        renderer2d.drawFilledTriangle(cx - s * 0.5f, cy - s * 0.7f, cx - s * 1.1f, cy + s * 0.4f,
                                      cx + s * 0.1f, cy + s * 0.4f, 0.462f, 0.431f, 0.384f,
                                      dimmed ? 0.4f : 0.95f);
        // Right peak (taller, catching light)
        renderer2d.drawFilledTriangle(cx + s * 0.3f, cy - s * 1.0f, cx - s * 0.3f, cy + s * 0.4f,
                                      cx + s * 0.9f, cy + s * 0.4f, 0.522f, 0.490f, 0.435f,
                                      dimmed ? 0.4f : 0.95f);
        // Snow caps
        renderer2d.drawFilledTriangle(cx + s * 0.3f, cy - s * 1.0f, cx + s * 0.05f, cy - s * 0.5f,
                                      cx + s * 0.55f, cy - s * 0.5f, 0.867f, 0.890f, 0.902f,
                                      dimmed ? 0.3f : 0.9f);
        renderer2d.drawFilledTriangle(cx - s * 0.5f, cy - s * 0.7f, cx - s * 0.7f, cy - s * 0.25f,
                                      cx - s * 0.3f, cy - s * 0.25f, 0.816f, 0.843f, 0.859f,
                                      dimmed ? 0.3f : 0.85f);
    }
    ...
    if (feature == map::FeatureType::Hills) {
        ...
        renderer2d.drawFilledCircle(cx - s * 0.5f, cy + s * 0.3f, s * 1.1f, 0.52f, 0.48f, 0.38f,
                                    ha);
        renderer2d.drawFilledCircle(cx + s * 0.4f, cy + s * 0.1f, s * 1.0f, 0.56f, 0.52f, 0.42f,
                                    ha);
        // Highlight on top
        renderer2d.drawFilledCircle(cx + s * 0.3f, cy - s * 0.2f, s * 0.5f, 0.62f, 0.58f, 0.48f,
                                    ha * 0.6f);
    }
    ...
    if (feature == map::FeatureType::Forest || feature == map::FeatureType::Jungle) {
        ...
        for (int t = 0; t < 3; ++t) {
            ...
            // Tree crown (triangle)
            renderer2d.drawFilledTriangle(tx, ty - ts * 1.4f, tx - ts * 0.8f, ty, tx + ts * 0.8f,
                                          ty, tR, tG, tB, fa);
            // Second layer (wider, overlapping)
            renderer2d.drawFilledTriangle(tx, ty - ts * 0.8f, tx - ts * 1.0f, ty + ts * 0.5f,
                                          tx + ts * 1.0f, ty + ts * 0.5f, tR * 0.85f, tG * 0.9f,
                                          tB * 0.85f, fa);
            // Trunk (small rect)
            renderer2d.drawFilledRect(tx - ts * 0.15f, ty + ts * 0.3f, ts * 0.3f, ts * 0.5f, 0.35f,
                                      0.22f, 0.10f, fa);
        }
    }
    ...
    if (feature == map::FeatureType::Marsh) {
        ...
        for (int i = -1; i <= 1; ++i) {
            ...
            renderer2d.drawLine(cx - s * 2.0f, ly, cx - s * 0.5f, ly - s * 0.4f, 1.5f, 0.2f, 0.4f,
                                0.6f, ma);
            renderer2d.drawLine(cx - s * 0.5f, ly - s * 0.4f, cx + s * 1.0f, ly + s * 0.3f, 1.5f,
                                0.2f, 0.4f, 0.6f, ma);
            renderer2d.drawLine(cx + s * 1.0f, ly + s * 0.3f, cx + s * 2.0f, ly, 1.5f, 0.2f, 0.4f,
                                0.6f, ma);
        }
    }
    ...
    if (feature == map::FeatureType::Oasis) {
        ...
        // Palm trunk
        renderer2d.drawLine(cx - s * 0.8f, cy + s * 0.5f, cx - s * 0.3f, cy - s * 0.8f, 2.0f, 0.40f,
                            0.28f, 0.12f, oa);
        // Palm fronds
        renderer2d.drawFilledTriangle(cx - s * 0.3f, cy - s * 0.8f, cx - s * 1.3f, cy - s * 0.3f,
                                      cx + s * 0.2f, cy - s * 0.4f, 0.15f, 0.45f, 0.10f, oa);
    }
    ...
    if (wonder != map::NaturalWonderType::None) {
        ...
        // Diamond shape
        renderer2d.drawFilledTriangle(cx, cy - s, cx - s * 0.7f, cy, cx + s * 0.7f, cy, 0.95f,
                                      0.82f, 0.22f, wa);
        renderer2d.drawFilledTriangle(cx, cy + s, cx - s * 0.7f, cy, cx + s * 0.7f, cy, 0.88f,
                                      0.72f, 0.18f, wa);
        // Glow ring
    ...
    if (improvement == map::ImprovementType::Canal) {
        ...
        renderer2d.drawFilledRect(cx - canalW * 0.5f, cy - hs * 0.6f, canalW, hs * 1.2f, 0.18f,
                                  0.42f, 0.68f, ca);
        // Stone banks on each side
        const float bankW = hs * 0.06f;
        const float bankA = dimmed ? 0.2f : 0.55f;
        renderer2d.drawFilledRect(cx - canalW * 0.5f - bankW, cy - hs * 0.6f, bankW, hs * 1.2f,
                                  0.50f, 0.48f, 0.42f, bankA);
        renderer2d.drawFilledRect(cx + canalW * 0.5f, cy - hs * 0.6f, bankW, hs * 1.2f, 0.50f,
                                  0.48f, 0.42f, bankA);
    }
    ...
    if (improvement != map::ImprovementType::None && improvement != map::ImprovementType::Road &&
        ...
        case map::ImprovementType::LumberMill:
            renderer2d.drawFilledRect(cx - s * 0.8f, iy - s * 0.3f, s * 1.6f, s * 0.6f, 0.50f,
                                      0.35f, 0.15f, ia);
            break;
        ...
        default:
            ...
            renderer2d.drawFilledTriangle(cx, iy - s, cx - s * 0.6f, iy, cx + s * 0.6f, iy, 0.6f,
                                          0.55f, 0.4f, ia);
            renderer2d.drawFilledTriangle(cx, iy + s, cx - s * 0.6f, iy, cx + s * 0.6f, iy, 0.55f,
                                          0.50f, 0.35f, ia);
            break;
        }
    }
    ...
    if (tileResource.isValid() && tileResource.value < aoc::sim::goodCount() &&
        ...
        case aoc::sim::GoodCategory::RawLuxury:
            ...
            renderer2d.drawFilledTriangle(cx, ry - s, cx - s * 0.7f, ry, cx + s * 0.7f, ry, rr, rg,
                                          rb, ra);
            renderer2d.drawFilledTriangle(cx, ry + s, cx - s * 0.7f, ry, cx + s * 0.7f, ry,
                                          rr * 0.8f, rg * 0.8f, rb * 0.8f, ra);
            break;
        case aoc::sim::GoodCategory::RawBonus:
            ...
            rb = 0.30f;
            // Bonus: small square
            renderer2d.drawFilledRect(cx - s * 0.8f, ry - s * 0.8f, s * 1.6f, s * 1.6f, rr, rg, rb,
                                      ra);
            break;
        default:
            break;
...
void MapRenderer::drawTerritoryBorders(vulkan_app::renderer::Renderer2D& renderer2d,
                                       const aoc::map::HexGrid& grid, const aoc::map::FogOfWar& fog,
                                       PlayerId viewingPlayer, const CameraController& camera,
    ...
    const int32_t minCol =
        cylindricalB ? (static_cast<int32_t>(topLeftX / xSpacing) - 1)
                     : std::max(0, static_cast<int32_t>(std::max(0.0f, topLeftX) / xSpacing) - 1);
    const int32_t maxCol =
        cylindricalB
            ? (static_cast<int32_t>(botRightX / xSpacing) + 1)
    ...
    for (int32_t row = minRow; row <= maxRow; ++row) {
        for (int32_t col = minCol; col <= maxCol; ++col) {
            ...
            if (tileVis == aoc::map::TileVisibility::Unseen) {
                continue;
            }
...
void MapRenderer::drawRiverEdges(vulkan_app::renderer::Renderer2D& renderer2d, uint8_t riverMask,
    ...
    for (int dir = 0; dir < 6; ++dir) {
        ...
        renderer2d.drawCapsule(x1, y1, x2, y2, 2.5f, 0.10f, 0.35f, 0.80f, 1.0f, 0.0f);
    }
}

void MapRenderer::drawToBuffer(DrawCommandBuffer& buffer, const aoc::map::HexGrid& grid,
                               const aoc::map::FogOfWar& fog, PlayerId viewingPlayer,
                               const CameraController& camera, uint32_t screenWidth,
                               uint32_t screenHeight) const {
    // Compute visible world-space bounds from camera
...
void MapRenderer::drawYieldLabels(vulkan_app::renderer::Renderer2D& renderer2d,
    ...
    const int32_t maxCol =
        cylindricalY ? (static_cast<int32_t>(botRightX / xSpacing) + 1)
    ...
    for (int32_t row = minRow; row <= maxRow; ++row) {
        for (int32_t col = minCol; col <= maxCol; ++col) {
            ...
            if (yields.food > 0) {
                totalIcons += yields.food;
            }
            if (yields.production > 0) {
                totalIcons += yields.production;
            }
            if (yields.gold > 0) {
                totalIcons += yields.gold;
            }
            if (yields.science > 0) {
                totalIcons += yields.science;
            }
            if (yields.culture > 0) {
                totalIcons += yields.culture;
            }
